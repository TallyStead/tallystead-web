...
type NetworkConfig = { canonical_url: string; internal_url: string | null; access_mode: string; trusted_proxy_cidrs: string[]; certificate_mode: string; dns_provider: string | null; dns_zone: string | null; cloudflare_configured: boolean; acme_email: string | null; internet_exposure_confirmed: boolean };
...
type Diagnostic = { effective_url: string; scheme: string; host: string; source_address: string | null; forwarded_headers_trusted: boolean };
...
export default function ServerPage() {
  ...
  async function stage(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await request<Network>("/v1/system/network/stage", { method: "PUT", body: JSON.stringify({ canonical_url: form.get("canonical_url"), internal_url: form.get("internal_url") || null, access_mode: form.get("access_mode"), trusted_proxy_cidrs: String(form.get("trusted_proxy_cidrs") || "").split(/[\n,]+/).map((value) => value.trim()).filter(Boolean), certificate_mode: form.get("certificate_mode"), dns_provider: form.get("certificate_mode") === "cloudflare_dns" ? "cloudflare" : null, dns_zone: form.get("dns_zone") || null, cloudflare_api_token: form.get("cloudflare_api_token") || null, acme_email: form.get("acme_email") || null, internet_exposure_confirmed: form.has("internet_exposure_confirmed") }) });
  ...
  return <div className="grid">
    ...
    <Panel span={5}><p className="eyebrow">Recovery</p><h2>Last known good</h2><div className="row"><div><b>Active revision</b><small>{network.active.canonical_url}</small></div><Pill>{network.revision}</Pill></div><button className="button full-width" disabled={busy || !network.last_known_good} onClick={() => { if (window.confirm("Restore the last known-good network and certificate configuration?")) void action("/v1/system/network/rollback", "Last known-good network configuration restored."); }}>Restore previous configuration</button><button className="button full-width" disabled={busy} onClick={diagnose}>Test forwarded request identity</button>{diagnostic && <div className="empty-inline top-space"><b>{diagnostic.effective_url}</b><br />Source: {diagnostic.source_address ?? "unknown"}<br />Forwarded headers: {diagnostic.forwarded_headers_trusted ? "trusted" : "ignored"}</div>}<div className="row top-space"><div><b>Latest backup</b><small>{status?.latest_backup_at ? new Date(status.latest_backup_at).toLocaleString() : "No backup recorded"}</small></div><Pill tone={status?.latest_backup_status === "succeeded" ? "green" : "amber"}>{status?.latest_backup_status ?? "Not run"}</Pill></div></Panel>
  </div>;
}
